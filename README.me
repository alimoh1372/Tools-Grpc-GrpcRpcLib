...
.NET 8 SDK: Download from Microsoft .NET.
...
gRPC Tools: For testing gRPC services, install grpcurl:go install github.com/fullstorydev/grpcurl/...@latest
...
Ensure appsettings.json Files:

Create or update appsettings.json in both GrpcRpcLib.Consumer and GrpcRpcLib.Publisher projects (see Configuration for details).
Ensure the files are copied to the output directory by adding to .csproj:<ItemGroup>
...
Apply Database Migrations (for Sqlite or SqlServer):
...
Configuration
Consumer Configuration
The Consumer service is configured via appsettings.json in the GrpcRpcLib.Consumer project. Example configuration (see full example):
{
  "GrpcConsumer": {
    "Host": "localhost",
    "Port": 5000,
...
MessageStore: See Message Store Configuration.

Publisher Configuration
The Publisher service is configured via appsettings.json in the GrpcRpcLib.Publisher project. Example configuration (see full example):
{
  "GrpcPublisher": {
...
Example appsettings.json for the Consumer project:
{
  "GrpcConsumer": {
    "Host": "localhost",
    "Port": 5000,
...
Example appsettings.json for the Publisher project:
{
  "GrpcPublisher": {
    "TargetHost": "localhost",
    "TargetPort": 5000
...
The Consumer should listen on http://localhost:5000 (check console output for Now listening on: http://localhost:5000).
...
Test gRPC Service:Use grpcurl to verify the Consumer service:
...
Send Test Messages:Example test code in GrpcRpcLib.Test.Publisher/Program.cs:
...
class Program
{
    ...
    {
        var publisher = // Resolve GrpcPublisher from DI;
        var message = new MessageEnvelope
        {
            Id = Guid.NewGuid(),
            Type = "TestMessage",
            CorrelationId = Guid.NewGuid().ToString(),
            Payload = Encoding.UTF8.GetBytes("Hello, gRPC!"),
            CreatedAt = DateTime.UtcNow,
            Status = "ResponsePending"
        };
        await publisher.SendAsync(message, "TestMethod", "TestCaller");
...
HTTP/2 Errors (e.g., HTTP_1_1_REQUIRED):

Ensure Kestrel is configured for HTTP/2 in GrpcConsumerServerExtensions.cs:services.Configure<KestrelServerOptions>(options =>
{
...
Check that the Consumer is running and listening on the correct port:netstat -an | findstr 5000


Open port 5000 in the firewall:netsh advfirewall firewall add rule name="Allow gRPC 5000" dir=in action=allow protocol=TCP localport=5000
...
Check logs for detailed errors (enable Information log level).
...
.NET 8 SDK: از Microsoft .NET دانلود کنید.
...
ابزارهای gRPC: برای تست سرویس‌های gRPC، ابزار grpcurl را نصب کنید:go install github.com/fullstorydev/grpcurl/...@latest
...
بررسی فایل‌های appsettings.json:

فایل appsettings.json را در پروژه‌های GrpcRpcLib.Consumer و GrpcRpcLib.Publisher ایجاد یا به‌روزرسانی کنید (جزئیات در پیکربندی).
مطمئن شوید فایل‌ها به دایرکتوری خروجی کپی می‌شوند:<ItemGroup>
  <None Update="appsettings.json">
    <CopyToOutputDirectory>Always</CopyToOutputDirectory>
  </None>
</ItemGroup>
...
اعمال Migrationهای دیتابیس (برای Sqlite یا SqlServer):
dotnet ef migrations add InitialCreate --project GrpcRpcLib.Shared --startup-project GrpcRpcLib.Consumer
dotnet ef database update --project GrpcRpcLib.Shared --startup-project GrpcRpcLib.Consumer
...
پیکربندی
پیکربندی Consumer
سرویس Consumer از طریق appsettings.json در پروژه GrpcRpcLib.Consumer پیکربندی می‌شود. نمونه تنظیمات (نمونه کامل):
{
  "GrpcConsumer": {
    "Host": "localhost",
    "Port": 5000,
...
MessageStore: به پیکربندی Message Store مراجعه کنید.

پیکربندی Publisher
سرویس Publisher از طریق appsettings.json در پروژه GrpcRpcLib.Publisher پیکربندی می‌شود. نمونه تنظیمات (نمونه کامل):
{
  "GrpcPublisher": {
    "TargetHost": "localhost",
    "TargetPort": 5000
...
نمونه appsettings.json برای Consumer
نمونه فایل appsettings.json برای پروژه Consumer:
{
  "GrpcConsumer": {
    "Host": "localhost",
    "Port": 5000,
...
نمونه appsettings.json برای Publisher
نمونه فایل appsettings.json برای پروژه Publisher:
{
  "GrpcPublisher": {
    "TargetHost": "localhost",
    "TargetPort": 5000
...
Consumer باید روی http://localhost:5000 listen کند (خروجی کنسول را بررسی کنید: Now listening on: http://localhost:5000).
...
تست سرویس gRPC:با استفاده از grpcurl سرویس Consumer را بررسی کنید:
grpcurl -plaintext localhost:5000 list
...
ارسال پیام‌های آزمایشی:نمونه کد تست در GrpcRpcLib.Test.Publisher/Program.cs:
using GrpcRpcLib.Publisher.Services;
using GrpcRpcLib.Shared.Entities.Models;
using System;
using System.Text;
...
خطاهای HTTP/2 (مثل HTTP_1_1_REQUIRED):

مطمئن شوید Kestrel در GrpcConsumerServerExtensions.cs برای HTTP/2 تنظیم شده است:services.Configure<KestrelServerOptions>(options =>
{
    options.ListenAnyIP(grpcConfiguration.Port, listenOptions =>
    {
        listenOptions.Protocols = HttpProtocols.Http2;
...
بررسی کنید که Consumer روی پورت درست listen می‌کند:netstat -an | findstr 5000


پورت 5000 را در فایروال باز کنید:netsh advfirewall firewall add rule name="Allow gRPC 5000" dir=in action=allow protocol=TCP localport=5000
...
لاگ‌ها را برای خطاهای دقیق بررسی کنید (سطح لاگ Information را فعال کنید).
